...
import org.firstinspires.ftc.teamcode.challenge201617VelocityVortex.duval.subClasses.LauncherControlFull;

/**
 * Created by johnduval on 1/11/17.
 */
@Autonomous(name = "Red Wall Launch Beacons", group = "DUVAL")
@Disabled

public class RedWallBeaconsEncoders extends LinearOpMode {
...
//    DcMotor rightLauncherMotor;
  ...
  //  DcMotor beltMotor;
    ...
    final static int launcherTimeB = 1000; //time belt will run to pick up the particles and launch them
...
*/
    final static double MOVEDISTANCE1 = 11; //wall to launching point
    final static double MOVEDISTANCE2 = 47.5; //launching point to white line
    final static double MOVEDISTANCE3 = 23.4; //white line to beacon to read color
    final static double MOVEDISTANCE4 = 10; // goes back to raise arms
    final static double MOVEDISTANCE5 = 8; //goes forward to press beacon button
    final static double MOVEDISTANCE6 = 8; //reverses after pressing beacon

    final static double TURNDISTANCE1 = 5.91; //turn from launching position to position to move to white line
    final static double TURNDISTANCE2 = 6.70; //turn from white line to parallel with white line
...
//resets encoders
      ...
      //  rightLauncherMotor = hardwareMap.dcMotor.get("right_launcher");
        ...
        leftArm.setPosition(leftBeaconArmDownPosition);
        rightArm.setPosition(rightBeaconArmDownPosition);
...
//turns LEFT from launching position to angle to move to white line
        movement = 3;
...
//goes to white line
        ...
        drive(COUNTS, movement);
//turns LEFT to go straight on line
        movement = 3;
...
//goes straight on line to get beacon color
        movement = 1;
        ...
        if (colorSensor.blue() > blueArmRaiseTheshold) {
            beaconBLUE = true;
            beaconRED = false;
        }
        else if (colorSensor.red() > redArmRaiseThreshold) {
            beaconBLUE = false;
            beaconRED = true;
        }
        else {
        ...
        //moves robot back from beacon
        movement = 2;
        COUNTS = calculateRotations(MOVEDISTANCE4);
        drive(COUNTS, movement);
//raises beacon arm if red
        if (beaconRED) {
            rightArm.setPosition(rightBeaconArmUpPosition);
        }
//raises beacon arm if blue
        else if (beaconBLUE){
            leftArm.setPosition(leftBeaconArmUpPosition);
        }
        ...
        //robot goes to hit beacon
        movement = 1;
        COUNTS = calculateRotations(MOVEDISTANCE5);
        drive(COUNTS, movement);

        //go back after hitting beacon
        movement = 2;
        COUNTS = calculateRotations(MOVEDISTANCE6);
        drive(COUNTS, movement);
        ...
        /*movement = 4;
        COUNTS = calculateRotations(TURNDISTANCE3);
        drive(COUNTS, movement);*/
